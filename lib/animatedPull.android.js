...
class AnimatedPTR extends React.Component {
    ...
    static propTypes = {
        ...
        minLoadDistance: React.PropTypes.number,
        /**
    ...
    //     if (this.state.isRefreshing !== props.isRefreshing) {
    ...
    _handleStartShouldSetPanResponder(e, gestureState) {
        return !this.state.isScrollFree && !this.state.loading
    }
    ...
    render() {
        ...
        const minLoadDistance = this.props.minLoadDistance
        let onScrollEvent = (event) => {
            if (onScroll) {
                onScroll(event)
            }
            const {layoutMeasurement, contentSize, contentOffset} = event.nativeEvent
            this.state.scrollY.setValue(contentOffset.y)
            if (!this.props.loadDone && contentOffset.y + layoutMeasurement.height + minLoadDistance >= contentSize.height &&
                (this.loadCount === 0 || this.loadCount === this.contentChange - 1) &&
                !this.state.loading && onLoad) {
                this.setState({isScrollFree: false, loading: true})
                this.loadCount++//计算加载次数
                onLoad(() => {
                    this.setState({isScrollFree: true, loading: false})
                })
            }
        }
        ...
        return (
            <View style={{flex: 1, backgroundColor: this.props.contentBackgroundColor}}
                ...
                <ScrollView ref='PTR_ScrollComponent'
                            ...
                            onResponderRelease={() => {
                                this.onScrollRelease.bind(this)
                            }}
                >
                    <Animated.View style={{
